...
async function fetchCompletedIssues(lookbackDays = 7) {
  ...
  const query = `
    query CompletedIssues($since: DateTimeOrDuration!) {
      issues(
        filter: { state: { type: { eq: "completed" } }, completedAt: { gte: $since } }
        first: 100
        orderBy: updatedAt
      ) {
        nodes {
  ...
  while (hasNextPage) {
    const paginatedQuery = cursor ? query.replace("first: 100", `first: 100, after: "${cursor}"`) : query;
  ...
  for (const issue of allIssues) {
    ...
    byTeam[team].push({
      ...
      labels: issue.labels.nodes.map((l) => l.name).filter((l) => !SKIP_LABELS.includes(l)),
    });
  }
